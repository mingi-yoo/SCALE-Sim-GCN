...
def gen_all_traces(
        ...
        dram_adjacency_trace_file = "dram_adjacency_read.csv"
    ):
...
def gen_max_bw_numbers( dram_ifmap_trace_file, dram_filter_trace_file,
    ...
    if data_flow == 'gcn':
        max_dram_adjacency_bw = 0
        num_bytes = 0
        max_dram_adjacency_clk = ""
        f = open(dram_adjacency_trace_file, 'r')

        for row in f:
            clk = row.split(',')[0]
            num_bytes = len(row.split(',')) - 2

            if max_dram_adjacency_bw < num_bytes:
                max_dram_adjacency_bw = num_bytes
                max_dram_adjacency_clk = clk
    ...
    if data_flow == 'gcn':
        log += str(max_dram_adjacency_bw) + ",\t"
    log += str(max_sram_read_bw) + ",\t"
...
def gen_bw_numbers( dram_ifmap_trace_file, dram_filter_trace_file,
    ...
    if data_flow == 'gcn':
        num_dram_adjacency_bytes = 0
        f = open(dram_adjacency_trace_file, 'r')
        first = True

        for row in f:
            num_dram_adjacency_bytes += len(row.split(',')) - 2
        ...
        stop_clk = clk
        detailed_log += str(start_clk) + ",\t" + str(stop_clk) + ",\t" + str(num_dram_adjacency_bytes) + ",\t"
        f.close()
        if clk > max_clk:
            max_clk = clk
    ...
    if data_flow == 'gcn':
        dram_adjacency_bw = num_dram_activation_bytes / delta_clk
