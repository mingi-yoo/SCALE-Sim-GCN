...
class scale:
    ...
    def parse_config(self):
       ...
       # config_filename = "./scale.cfg"
        ...
        if self.dataflow == 'gcn' or self.dataflow == 'gcn2':
            ...
            if len(adjacency_sram) > 1:
                self.asram_max = filter_sram[1].strip()

            adjacency_offset = config.get(arch_sec, 'AdjacencyOffset')
            self.adjacency_offset = int(adjacency_offset.strip())
    ...
    def run_once(self):
        ...
        elif self.dataflow == 'gcn' or self.dataflow == 'gcn2':
        ...
        if self.dataflow == 'gcn':
            offset_list = [self.ifmap_offset, self.filter_offset, self.ofmap_offset, self.adjacency_offset]

            r.run_net(  ifmap_sram_size  = int(self.isram_min),
                        filter_sram_size = int(self.fsram_min),
                        ofmap_sram_size  = int(self.osram_min),
                        adjacency_sram_size = int(self.asram_min),
