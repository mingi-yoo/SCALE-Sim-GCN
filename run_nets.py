...
def run_net( ifmap_sram_size=1,
    ...
    if data_flow == 'gcn':
        bw.write("IFMAP SRAM Size,\tFilter SRAM Size,\tOFMAP SRAM Size,\tADJMAP SRAM Size,\tConv Layer Num,\tDRAM IFMAP Read BW,\tDRAM Filter Read BW,\tDRAM OFMAP Write BW,\tSRAM Read BW,\tSRAM OFMAP Write BW,\tADJMAP SRAM Size, \n")
        maxbw.write("IFMAP SRAM Size,\tFilter SRAM Size,\tOFMAP SRAM Size,\tADJMAP SRAM Size,\tConv Layer Num,\tMax DRAM IFMAP Read BW,\tMax DRAM Filter Read BW,\tMax DRAM OFMAP Write BW,\tMax SRAM Read BW,\tMax SRAM OFMAP Write BW,\tMax ADJMAP SRAM Size,\n")
        ...
        detailed_log = "Layer," +\
                     ...
                     "\tDRAM_ADJMAP_start,\tDRAM_ADJMAP_stop,\tDRAM_ADJMAP_bytes" + \
    ...
    if data_flow == 'gcn':
        ...
        for row in param_file:
           ...
           # print(len(elems))
            ...
            adjacency_h = int(elems[5])
            adjacency_w = int(elems[6])
            
            ifmap_base  = offset_list[0]
            filter_base = offset_list[1]
            ofmap_base  = offset_list[2]
            adjacency_base = offset_list[3]

            bw_log = str(ifmap_sram_size) +",\t" + str(filter_sram_size) + ",\t" + str(ofmap_sram_size) + ",\t" +  str(adjacency_sram_size) + ",\t" + name + ",\t"
            max_bw_log = bw_log
            detailed_log = name + ",\t"
            ...
            bw_str, detailed_str, util, clk =  \
                tg.gen_all_traces(  array_h = array_h,
                                    ...
                                    adjacency_sram_size = adjacency_sram_size,
                                    ...
                                    dram_adjacency_trace_file = net_name + "_" + name + "_dram_adjacency_read.csv"
                                )
