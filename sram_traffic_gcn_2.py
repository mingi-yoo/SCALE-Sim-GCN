...
def sram_traffic(
    ...
    # Variables to calculate folds in runtime (X * W)
    num_h_fold = math.ceil(filt_h/dimension_rows)
    num_v_fold = math.ceil(filt_w/dimension_cols)
    ...
    while (ifmap_done == False) or (filt_done == False):
        ...
        for r in range(dim_rows):
            ...
            if (row_clk_offset[r] >= 0):
                ...
                ifmap_addr = row_base_addr[r] + inc
            ...
            i += 1
            #if i < 10:
               # print(str(r) + " " + str(row_clk_offset[r]) + "\n")
            if (row_clk_offset[r] > 0) and (row_clk_offset[r] % filt_h == 0): #Compeleted MAC for one OFMAP px
                ...
                if ofmap_idx < ifmap_h:
                    row_clk_offset[r] = 0

                    row_base_addr[r] = ofmap_idx
        ...
        for c in range(dim_cols):
            if(col_clk_offset[c] >= 0):
                filt_addr = col_base_addr[c] + filt_base
                filt_read += ", "
                cols_used += 1
            else:
                filt_read += ", "
            ...
            if(col_clk_offset[c] > 0) and (col_clk_offset[c] % filt_h == 0):
